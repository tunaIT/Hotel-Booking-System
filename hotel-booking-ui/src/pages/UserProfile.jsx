...
const UserProfile = () => {
  ...
  useEffect(() => {
    // Only fetch if we have both username and id (preventing "undefined" ID errors)
    if (user?.username && user?.id) {
      fetchMyData();
    } else if (user && !user.id) {
      toast.error('Session is outdated. Please log out and log in again.', { duration: 4000 });
      setLoading(false);
    }
  }, [user]);
